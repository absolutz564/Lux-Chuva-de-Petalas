...
public class BubbleUI : MonoBehaviour
{
    ...
    private RectTransform spawnParent; // painel/canvas onde as bolhas s�o criadas
    ...
    // Chame esta fun��o para trocar o sprite aleatoriamente
    public void AssignRandomSprite()
    {
        if (targetImage == null)
        {
            Debug.LogWarning("Target Image n�o atribu�da!");
            return;
        }
    ...
    void Awake()
    {
        ...
        // se productImage n�o foi setado, tenta achar um filho Image (diferente da bolha)
    ...
    Sprite GetScoreSprite(int value)
    {
        ...
        return null; // ou algum sprite padr�o
    }
    ...
    public void BurstAndRelease()
    {
        ...
        // 4) spawn popup de pontua��o na mesma posi��o e faz ele sumir em 2s
        if (scorePopupPrefab != null)
        {
            ...
            // configura queda + dura��o 2s
            popup.SetActive(true); // garante que est� ativo
        ...
        // 2) opcional: spawn de explos�o (visual) na posi��o da bolha
        if (explosionPrefab != null && spawnParent != null)
        {
            ...
            if (vfxRt != null)
            {
                vfxRt.position = rt.position; // mesma posição da bolha
        ...
        if (productImage != null)
        {
            // garante que n�o capture cliques ap�s soltar
            productImage.raycastTarget = false;

            // guarda posi��o mundial atual
            ...
            productImage.transform.SetParent(newParent, true); // preserva posi��o mundial
            ...
            // adiciona FallUI para simular queda com for�a (objeto pesado)
            ...
            fall.gravity = 3000f;   // acelera��o
            fall.lifetime = 5f;     // destr�i depois se cair fora da tela
            fall.angularVelocity = Random.Range(-120f, 120f); // rota��o opcional
        }

        // 5) destruir a bolha (o filho j� foi reparentado, ent�o n�o ser� destru�do)
        Destroy(gameObject);
    }
}
